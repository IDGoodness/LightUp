...
import { upcomingEventsData, pastEventsData } from '../data/mockData';
    ...
    <div className="events-page">
      ...
      <header className="relative bg-cover bg-center bg-no-repeat min-h-[50vh] flex items-center justify-center text-center py-24 px-6" style={{ backgroundImage: `url(${homepageImg})` }}>
        ...
        <div className="relative z-20 max-w-[850px] animate-fade-in">
          <p className="font-heading font-bold uppercase tracking-[0.15em] text-primary mb-6">EVENTS & CONFERENCES</p>
          <h1 className="text-white mb-6 font-heading font-extrabold text-[2.5rem] sm:text-[3.5rem] md:text-[4rem] leading-tight">
            Gather, Grow, and Experience <span className="text-gradient-orange">God's Presence</span>
          </h1>
          <p className="max-w-[750px] mx-auto text-[1.1rem] text-text-dimmed leading-relaxed">
            We gather in fellowship, training, and spiritual warfare, aligning ourselves as kingdom representatives. Explore our schedule of hybrid seminars, online coordinate meetings, and regional congresses.
          </p>
        </div>
      </header>
      ...
      <section className="py-24 bg-bg-dark">
        <div className="w-full max-w-[1200px] mx-auto px-6 md:px-8">
          <h2 className="text-3xl font-heading font-bold mb-10 pb-4 border-b border-white/5 text-text-white">Upcoming events</h2>
          
          <div className="flex flex-col gap-6">
            {upcomingEventsData.map((event) => {
              const isRegistered = registeredEvents.includes(event.id);
              return (
                <div className="flex flex-col md:flex-row bg-card-dark-lighter rounded-lg overflow-hidden border border-primary/20 transition-all duration-300 hover:scale-[1.01] hover:shadow-[0_10px_30px_rgba(0,0,0,0.4)]" key={event.id}>
                  ...
                  <div className="p-6 md:p-8 flex flex-col md:flex-row flex-1 justify-between items-start md:items-center gap-6 md:gap-8">
                    <div className="max-w-[500px]">
                      ...
                      <h3 className="text-2xl font-heading font-bold text-text-white mb-2">{event.title}</h3>
                      <p className="text-text-dimmed text-[0.95rem] mb-4 leading-relaxed">{event.description}</p>
      ...
      <section className="py-24 bg-bg-black">
        <div className="w-full max-w-[1200px] mx-auto px-6 md:px-8">
          <h2 className="text-3xl font-heading font-bold mb-10 pb-4 border-b border-white/5 text-text-white">Past Events</h2>

          <div className="flex flex-col gap-6">
            {pastEventsData.map((event) => (
              <div className="flex flex-col md:flex-row bg-card-dark-lighter rounded-lg overflow-hidden border border-primary/20 transition-all duration-300 hover:scale-[1.01] hover:shadow-[0_10px_30px_rgba(0,0,0,0.4)] opacity-85" key={event.id}>
                <div className="w-full md:w-[260px] h-[200px] md:h-auto min-h-[180px] relative shrink-0">
      ...
      <section className="py-24 bg-gradient-to-br from-accent-deep-purple to-bg-dark text-center">
        <div className="w-full max-w-[800px] mx-auto px-6 md:px-8">
          <h2 className="text-3xl sm:text-4xl font-heading font-bold text-text-white mb-5 leading-tight">NEVER MISS AN EVENT</h2>
          <p className="text-text-dimmed mb-10 text-lg leading-relaxed">
            Subscribe to receive email notifications about our weekly intercessory coordinates, regional training schedules, and special congress announcements.
          </p>
          <div className="flex justify-center">
            <button 
              onClick={() => {
                const email = prompt('Enter your email to receive event notifications:');
                if (email) alert(`Thank you! ${email} has been subscribed to Event Alerts.`);
              }}
      ...
      {/* Video Modal Overlay */}
      {activeVideo && (
        <div className="fixed inset-0 bg-black/85 z-[2000] flex items-center justify-center p-8" onClick={closeVideo}>
          <div className="bg-bg-dark rounded-lg max-w-[800px] w-full overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.5)] border border-primary/20 relative" onClick={(e) => e.stopPropagation()}>
            <button className="absolute top-4 right-4 bg-transparent border-none text-text-white cursor-pointer z-10" onClick={closeVideo} aria-label="Close highlights player">
              <X size={24} />
            </button>
