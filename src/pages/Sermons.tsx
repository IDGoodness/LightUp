    ...
    <div className="sermons-page">
      ...
      <header className="relative bg-cover bg-center bg-no-repeat min-h-[50vh] flex items-center justify-center text-center py-24 px-6" style={{ backgroundImage: `url(${homepageImg})` }}>
        ...
        <div className="relative z-20 max-w-[850px] animate-fade-in">
          <p className="font-heading font-bold uppercase tracking-[0.15em] text-primary mb-6">SOUND DOCTRINE & TEACHINGS</p>
          <h1 className="text-white mb-6 font-heading font-extrabold text-[2.5rem] sm:text-[3.5rem] md:text-[4rem] leading-tight">
            MESSAGES THAT <span className="text-gradient-orange">INSPIRES FAITH</span>
          </h1>
          <p className="max-w-[750px] mx-auto text-[1.1rem] text-text-dimmed leading-relaxed">
            Listen, watch, and download our library of biblical resources. Every message is designed to anchor your soul, teach kingdom principles, and unlock spiritual maturity.
          </p>
        </div>
      </header>
      ...
      <section className="py-24 bg-bg-dark">
        <div className="w-full max-w-[1200px] mx-auto px-6 md:px-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-12 border-b border-white/5 pb-6">
            <h2 className="text-3xl font-heading font-bold text-text-white">Browse our sermons</h2>
          ...
          {filteredSermons.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mt-16">
              {filteredSermons.map((sermon) => (
                ...
                >
                  <img src={sermon.thumbnail} alt={sermon.title} className="w-full h-full object-cover opacity-65 transition-all duration-300 group-hover:opacity-85 group-hover:scale-103" />
                  <div className="absolute inset-0 p-6 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex flex-col justify-end h-full">
                    <h3 className="text-[1.2rem] font-bold text-text-white mb-1">{sermon.title}</h3>
                    <span className="text-[0.85rem] text-accent-orange mb-3">by {sermon.speaker}</span>
            ...
            <div className="text-center py-16 text-text-dimmed">
              <h3 className="text-xl font-semibold mb-2">No sermons found matching "{searchQuery}"</h3>
              <p className="text-sm">Try checking your spelling or looking for other keywords.</p>
            </div>
          )}
        </div>
      </section>
      ...
      {activeVideo && (
        <div className="fixed inset-0 bg-black/85 z-[2000] flex items-center justify-center p-8" onClick={closeVideo}>
          <div className="bg-bg-dark rounded-lg max-w-[800px] w-full overflow-hidden shadow-[0_20px_40px_rgba(0,0,0,0.5)] border border-primary/20 relative" onClick={(e) => e.stopPropagation()}>
            <button className="absolute top-4 right-4 bg-transparent border-none text-text-white cursor-pointer z-10" onClick={closeVideo} aria-label="Close sermon player">
              <X size={24} />
            </button>
